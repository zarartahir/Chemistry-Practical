...
namespace UnitySimpleLiquid
{
    ...
    public class LiquidContainer : MonoBehaviour
    {
        ...
        {
            MaterialInstance.color = liquidColor;
            GameObject go = GameObject.FindGameObjectWithTag("ElementText");
            element = go.GetComponent<Text>();
            go = GameObject.FindGameObjectWithTag("AcidicText");
            acidic = go.GetComponent<Text>();
            go = GameObject.FindGameObjectWithTag("BasicText");
            basic = go.GetComponent<Text>();
            go = GameObject.FindGameObjectWithTag("NeutralText");
            neutral = go.GetComponent<Text>();
        }

        public void Reset()
        {
            FillAmountPercent = 0;
            GameObject[] go = GameObject.FindGameObjectsWithTag("Blue Paper");
            for (int i=0;i<go.Length;i++)
            {
                Destroy(go[i]);
            }
            go = GameObject.FindGameObjectsWithTag("Red Paper");
            for (int i = 0; i < go.Length; i++)
            {
                Destroy(go[i]);
            }
        ...
        private void OnTriggerEnter(Collider other)
        {
            ...
            if (other.tag == "HCL" || other.tag == "NaOH" || other.tag == "HNO" || other.tag == "KOH")
            {
                currentElement = other.tag;
                FillAmountPercent = 0.75f;
                element.text = "Element = " + currentElement;
                acidic.text = "";
                basic.text = "";
                Destroy(other.gameObject);
            }

            if (other.tag=="Blue Paper")
            {
                if (currentElement == "HCL")
                {
                    other.transform.GetComponent<Renderer>().material.color = Color.red;
                    acidic.text = "Acidic = Yes";
                }
                if (currentElement == "NaOH")
                {
                    acidic.text = "Acidic = No";
                }
                if (currentElement == "HNO")
                {
                    other.transform.GetComponent<Renderer>().material.color = Color.red;
                    acidic.text = "Acidic = Yes";
                }
                if (currentElement == "KOH")
                {
                    acidic.text = "Acidic = No";
                }
            }

            if (other.tag == "Red Paper")
            {
                if (currentElement == "HCL")
                {
                    basic.text = "Basic = No";
